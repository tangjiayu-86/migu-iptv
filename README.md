...
### v1.9.0 (2026-05-30)
- 🔒 **重构访问密码鉴权**：`/api/*` 由校验可伪造的 `Referer` 头改为与页面一致的**路径密码**校验（密码模式下需经 `/<密码>/api/...` 访问），管理后台前端同步适配；未授权统一返回 403，未知接口返回 404，非 GET/POST 返回 405
- ⚡ **移除全局请求串行锁**：旧实现用一个全局 `loading` 标志把所有 HTTP 请求逐个串行处理，一次慢抓取/拉取会阻塞包括频道跳转在内的全部请求；现各请求独立处理。POST 请求体读取加 try/catch，客户端中途断开连接时返回 400 而非拖垮进程（并新增进程级异常兜底，避免单个异常导致整个服务退出）
- 🛡️ **更新流程加单飞锁**：定时刷新、启动初始化与后台保存外部源可能并发调用 `update()`，并发改写同一批临时文件会损坏 `interface.txt` / `playback.xml`；现串行化执行，避免播放列表损坏
...
- ♻️ **恢复 `refreshToken` 开关**：每月一次的咪咕 token 刷新（可能导致封号）现可通过配置 / 环境变量关闭
